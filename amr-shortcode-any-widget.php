...
function amr_spice_get_widget_id($widget_instance) {
    ...
    if ($widget_instance->number=="__i__") {
		...
		//if (is_plugin_active('classic-widgets/classic-widgets.php')) {
			echo 'or maybe with id [do_widget id='.esc_html($widget_instance->id).'] - incorrect unless Classic Widgets is active</p>';
		//}
		//else {   */
...
global $wp_registered_widgets, $_wp_sidebars_widgets, $wp_registered_sidebars;
	...
	if (empty ($wp_registered_sidebars[$widget_area])) {
		echo '<br/>Widget area "'.esc_html($widget_area).'" not found. Registered widget areas (sidebars) are: <br/>';
		foreach ($wp_registered_sidebars as $area=> $sidebar) echo esc_html($area).'<br />';
	}
		...
		$wid = array_intersect ($_wp_sidebars_widgets[$sidebarid], $widget_ids );
			...
			if (isset($debug)) {  // only do this in debug mode
				if (!isset($_wp_sidebars_widgets[$sidebarid]))
					$output =  '<p>Error: Sidebar "'.esc_html($sidebar).'" with sidebarid "'.esc_html($sidebarid).'" is not defined.</p>'; 
				 // shouldnt happen - maybe someone running content filters on save
				else 
					$output =  '<p>Error: Sidebar "'.esc_html($sidebar).'" with sidebarid "'.esc_html($sidebarid).'" is empty (no widgets)</p>'; 
			}		
		}
